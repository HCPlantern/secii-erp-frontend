    ...
    <el-card v-for="(item,index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        <el-row>
          ...
          <el-col :span="8">
            <span><strong>收款总金额(元): </strong>{{item.totalAmount}}</span>
          </el-col>
        </el-row>
      </div>
      <div v-if="showAll[index]" style="margin-top: 15px">
        <div style="margin-bottom: 20px"><strong>详细收款条目:</strong></div>
          <el-table
              :data="item.collectionContent"
              stripe
              style="width: 100%"
...
<script>
import {collectionSheetApproval,createCollectionSheet} from '@/network/finance'
export default {
  name: "CollectionList",
  props:{
    list: Array,
